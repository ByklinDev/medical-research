...
namespace MedicalResearch.Domain.Services
{
    public class MedicineService(IUnitOfWork unitOfWork, IValidator<Medicine> medicineValidator, ILogger<MedicineService> logger) : IMedicineService
    {
        ...
        public async Task<List<Medicine>> GetMedicinesAsync(Query query)
        {
            try
            {
                return await unitOfWork.MedicineRepository.GetAllAsync(query);
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<Medicine>> GetMedicinesByNameAsync(Query query)
        {
            try
            {
            ...
            {
                logger.LogError(ex, "Medicines by name could not be retrieved: {message}", ex.Message);
                throw new DomainException("Error while retrieving Medicines by name");
            }
        }
    }
}
