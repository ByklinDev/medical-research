...
namespace MedicalResearch.Domain.Services
{
    public class VisitService(IUnitOfWork unitOfWork, ILogger<VisitService> logger) : IVisitService
    {
        ...
        public async Task<List<Visit>> GetVisitsAsync(Query query)
        {
            try
            {
                return await unitOfWork.VisitRepository.GetAllAsync(query);
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<Visit>> GetVisitsByNameAsync(Query query)
        {
            try
            {
            ...
            {
                logger.LogError(ex, "Visits could not be retrieved by name: {message}", ex.Message);
                throw new DomainException("Error while retrieving Visits by name");
            }
        }
    }
}
