...
namespace MedicalResearch.Domain.Services
{
    public class ClinicService(IUnitOfWork unitOfWork, IValidator<Clinic> clinicValidator, ILogger<ClinicService> logger) : IClinicService
    {
        ...
        public async Task<List<Clinic>> GetClinicsAsync(Query query)
        {
            try
            {
                return await unitOfWork.ClinicRepository.GetAllAsync(query);
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<Clinic>> GetClinicsByNameAsync(Query query)
        {
            try
            {
            ...
            {
                logger.LogError(ex, "Clinics by name could not be retrieved: {message}", ex.Message);
                throw new DomainException("Error while retrieving Clinics by name");
            }
        }
    }
}
